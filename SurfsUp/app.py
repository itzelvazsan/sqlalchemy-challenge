...
def tobs():
    ...
    session.close()
    act_station_year_list = [{"date": date, "tobs": tobs} for date, tobs in act_station_year]
    return jsonify(act_station_year_list)
...
def start(start):
    # Create our session (link) from Python to the DB
    session = Session(engine)
    results_start = session.query(func.min(Measurement.tobs), func.max(Measurement.tobs), func.avg(Measurement.tobs)).filter(Measurement.date >= start)
    session.close()
    start_data = {
        "Start Date": start,
        "Min Temp": results_start[0][0],
        "Avg Temp": results_start[0][1],
        "Max Temp": results_start[0][2]
    }
...
def end(start, end):
    # Create our session (link) from Python to the DB
    session = Session(engine)
    results_start_end = session.query(func.min(Measurement.tobs), func.avg(Measurement.tobs), func.max(Measurement.tobs)).filter(Measurement.date >= start).filter(Measurement.date <= end)
    session.close()
    start_end_data = {
        "Start Date": start,
        "End Date": end,
        "Min Temp": results_start_end[0][0],
        "Avg Temp": results_start_end[0][1],
        "Max Temp": results_start_end[0][2]
    }
